...
ggplot(data, aes(x = occupation)) +
  geom_bar() +
  facet_wrap(~ race, labeller = as_labeller(toupper)) +  # Convert facet labels to uppercase
  labs(
    title = "Occupation Distribution by Race",
    x = "Occupation",
    y = "Count"
  ) +
  theme_minimal() +
  theme(
    ...
    strip.text = element_text(size = 10, face = "bold")  # Increase font size and bold facet labels
  )
