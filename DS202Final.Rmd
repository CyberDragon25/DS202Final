...
```{r}
country_counts <- table(data$native_country)
country_props <- prop.table(country_counts)
other_countries <- names(country_props[country_props < 0.005 & names(country_props) != "United-States"])
other_countries
data$native_country_grouped <- ifelse(data$native_country %in% other_countries, "Other", data$native_country)

ggplot(data, aes(x = "", fill = native_country_grouped)) +
  geom_pie(width = 1) +
  coord_polar(theta = "y") +  # Convert bars to pie slices
  labs(title = "Distribution of Individuals by Native Country") +
  theme_void()
